...
export class MtFileVierwerComponent extends MtBaseComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  public LoadPDF(): Promise<any> {
    ...
    // let guid = null
    // const beilage = props.formular?.formularDokumentPool?.find(b => b.formularBeilage?.formularTyp.guid === props.formularTypGuid)
    ...
    // }
    // if (guid) {
    //   return props.projektService.LoadPDF(guid).then(data => {
    //     if (data) {
