...
const MELDEFORMULAR_SOLAR_ZH_AnlageStandort: IComponent = card_panel('Anlage-Standort', 'MELDEFORMULAR_SOLAR_ZH_AnlageStandort', [
   {
      type: 'panel',
      name: 'projekt_daten',
      ...
      children: [
         ...
         },
         {
            ...
            children: [
               ...
               { type: 'label', label: 'Nutzungszone(n)', classLayout: 'text-xs mt-2', }, //Platzhalter
               { type: 'label', field: 'ZONE', classLayout: '', }, //Platzhalter
            ]
         },
      ]
   },
...
const MELDEFORMULAR_SOLAR_ZH_Adressen: IComponent = card_panel('Adressen / Geschäftspartner', '', [
   {
      type: 'panel',
...
const MELDEFORMULAR_SOLAR_ZH_ThSolaranlage: IComponent = switch_hint_panel('Thermische Solaranlage', 'MELDEFORMULAR_SOLAR_ZH_ThSolaranlage', 'TH_ANLAGE', false, '(Warmwasser)', [
   // label('', true),
   checkBoxGroup([
      { label: 'Flachkollektoren', field: 'TH_FLACHKOLLEKTOREN' },
      { label: 'Röhrenkollektoren', field: 'TH_ROEHRENKOLLEKTOREN' },
      { label: 'Andere', field: 'TH_ANDERE' }, //Platzhalter
   ], { required: false }),
   label('Gesamtfläche der Anlage'),
   {
      type: 'input',
      dataType: 'float',
      field: 'TH_ABSORBERFLAECHE', //Platzhalter
      suffix: 'm²',
      max: 10,
   },
]);

const MELDEFORMULAR_SOLAR_ZH_PvSolaranlage: IComponent = switch_hint_panel('Photovoltaikanlage', 'MELDEFORMULAR_SOLAR_ZH_PvSolaranlage', 'PV_ANLAGE', true, '(Elektrizität)', [
   label('Gesamtfläche der Anlage'),
   {
      type: 'input',
      dataType: 'float',
      field: 'PV_ABSORBERFLAECHE', //If PV=Yes then use this Else TH
      suffix: 'm²',
      max: 10,
   },
]);
...
export const MELDEFORMULAR_SOLAR_ZH: ISchema = {
   type: 'panel',
   name: 'MELDEFORMULAR_SOLAR_ZH',
   label: 'Meldeformular Solar (ZH)',
   iconText: 'MFZH',
   pdfTemplate: 'DA5223B6-8170-4429-BC34-20FC750983E3',
   pdfFileName: label,
   guid: FORMULAR_TYP_MELDEFORMULAR_SOLAR_ZH,
   attribut: 'c785ec2a-fd3e-4c7c-9ef1-64948973e870',
  //  beilagen: [
  //     { guid: FORMULAR_TYP_SITUATIONSPLAN, titel: 'Situationsplan' },
  //     { guid: FORMULAR_TYP_GRUNDRISSPLAN, titel: 'Grundrissplan' }, //zusätzlich bei ZH
  //     { guid: FORMULAR_TYP_FASSADENPLAN, titel: 'Fassadenplan' },
  //  ],
  //  steps: [
  //     { step: 1, titel: 'Sperren', status: MELDEFORMULAR_SOLAR_ZH_Status.Sperren, target: 'MELDEFORMULAR_SOLAR_ZH_Unterschriften' },
  //     { step: 2, titel: 'Senden', status: MELDEFORMULAR_SOLAR_ZH_Status.Senden, target: 'MELDEFORMULAR_SOLAR_ZH_Unterschriften' },
  //     { step: 3, titel: 'Antwort erhalten', status: MELDEFORMULAR_SOLAR_ZH_Status.Gesendet, target: 'MELDEFORMULAR_SOLAR_ZH_Unterschriften' },
  //  ],
   classLayout: 'w-full',
   children: [
      MELDEFORMULAR_SOLAR_ZH_AnlageStandort,
      MELDEFORMULAR_SOLAR_ZH_Adressen,
      // MELDEFORMULAR_SOLAR_ZH_Gesuchsteller,
      // MELDEFORMULAR_SOLAR_ZH_Grundeigentümer,
      MELDEFORMULAR_SOLAR_ZH_ThSolaranlage,
      MELDEFORMULAR_SOLAR_ZH_PvSolaranlage,
      // MELDEFORMULAR_SOLAR_ZH_AnlageAuausfuehrung,
      // MELDEFORMULAR_SOLAR_ZH_Unterlagen,
      // MELDEFORMULAR_SOLAR_ZH_Unterschriften,
   ],
  ...
  //     if ((comp.required || comp.parentComp?.required) && sm.Values.FormStatus == MELDEFORMULAR_SOLAR_ZH_Status.InArbeit) {
  //        const mussfelder = { anzahl: 0, filled: 0 }
  //        sm.GetFormularMussfelder(mussfelder);
  //        if (mussfelder.anzahl === mussfelder.filled)
  //           sm.Schema.setStatus(sm, MELDEFORMULAR_SOLAR_ZH_Status.Sperren);
  //        else
  //           sm.Schema.setStatus(sm, MELDEFORMULAR_SOLAR_ZH_Status.InArbeit);
  //     }
  //  },
  //  setStatus(sm: SchemaManager, status: MELDEFORMULAR_SOLAR_ZH_Status) {
  //     if (sm.Values.FormStatus !== status) {
  //        sm.setValueSave('FormStatus', status);
  //     }
  //     sm.Schema.SetFormularStatus(status, MELDEFORMULAR_SOLAR_ZH_StatusText[status], true);
  //  },
